...
def create_tables():
    ...
    with db_connection() as conn:
        ...
        # Check if media_grid_size setting exists
        cursor.execute("SELECT value FROM settings WHERE key = 'media_grid_size'")
        if cursor.fetchone() is None:
            cursor.execute("INSERT INTO settings (key, value) VALUES (?, ?)", 
                           ('media_grid_size', 'medium'))
            print("Initialized 'media_grid_size' setting to: medium")
